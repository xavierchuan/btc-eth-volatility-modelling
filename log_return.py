...
# Plotting helpers
# -----------------------------
def plot_returns(df: pd.DataFrame, ticker: str, save_name: str | None = None):
    fig, ax = plt.subplots(figsize=(8, 3.5))
    ax.plot(df.index, df["LogReturn"], label=f"{ticker} Log Returns")
    ax.axhline(0, color="black", linewidth=0.8)
    ax.set_title(f"Daily Log Returns - {ticker}")
    ax.set_xlabel("Date")
...
def run_for_ticker(ticker: str):
    ...
    # Plot returns first
    plot_returns(df, ticker, save_name=f"{ticker}_returns.png")

    # Descriptive stats + JB test
    ...
    # Residual diagnostics for best (by AIC)
